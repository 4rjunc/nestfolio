...
import { zksync as zksync2 } from "viem/chains";
...
var WalletProvider = class {
  ...
  async getAllowace(tokenAddress, owner, spender) {
  ...
  async sendTransaction(req) {
    const txRequest = {
      ...
      chain: zksync2,
      kzg: void 0,
      data: req.data ? req.data : void 0,
      to: req.to ? req.to : void 0
    };
  ...
  async fetchPortfolio() {
    try {
      ...
      if (!portfolioData || !portfolioData.success) {
        ...
        throw new Error(
          `Failed to fetch portfolio: ${portfolioData?.error || "Unknown error"}`
        );
      }
  ...
  async fetchAllTokens() {
    try {
      ...
      if (!tokensData || tokensData.error || !tokensData.data) {
        ...
        throw new Error(
          `Failed to fetch all tokens: ${tokensData?.error || "Unknown error"}`
        );
      }
...
// src/environment.ts
var holdStationEnvSchema = z.object({
  HOLDSTATION_PRIVATE_KEY: z.string().min(1, "holdstation plugin requires private key")
});
async function validateHoldStationConfig(runtime) {
  try {
    const config = {
      HOLDSTATION_PRIVATE_KEY: runtime.getSetting("HOLDSTATION_PRIVATE_KEY") || process.env.HOLDSTATION_PRIVATE_KEY
    };
...
var HOLDSTATION_ROUTER_ADDRESS = "0xD1f1bA4BF2aDe4F47472D0B73ba0f5DC30E225DF";
...
var SwapAction = class {
  ...
  async swap(params) {
    ...
    const filters = tokens.filter((t) => {
      return params.inputTokenCA ? t.address === params.inputTokenCA : t.symbol === params.inputTokenSymbol?.toUpperCase();
    });
    if (filters.length !== 1) {
      throw new Error(
    ...
    if (!params.outputTokenCA || !params.outputTokenSymbol) {
      ...
      const filters2 = tokens2.filter((t) => {
        return params.outputTokenCA ? t.address === params.outputTokenCA : t.symbol === params.outputTokenSymbol?.toUpperCase();
      });
    ...
    elizaLogger2.info("--- Swap params:", params);
    ...
    const nonce = await this.walletProvider.getPublicClient().getTransactionCount({
...
var swapAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.log("Starting HoldStation Wallet TOKEN_SWAP handler...");
    const walletProvider = await initWalletProvider(runtime);
...
var index_default = holdstationPlugin;
export {
