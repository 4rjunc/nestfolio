...
__export(actions_exports, {
  ContractInteractionAction: () => ContractInteractionAction,
  contractInteractionAction: () => contractInteractionAction
});
...
var GELATO_RELAY_API_KEY = process.env.GELATO_RELAY_API_KEY || "";
var relay = new GelatoRelay();
async function executeSponsoredCall(client, abi, functionName, args, target) {
  try {
...
async function executeSponsoredCallERC2771(client, abi, functionName, args, target, user) {
  try {
    const chainId = await client.getChainId();
    const data = encodeFunctionData({
      abi,
...
// src/schemas.ts
var SupportedChainsSchema = z.enum([
  "mainnet",
  "sepolia",
  "goerli",
  "polygon",
...
var ContractInteractionSchema = z.object({
  abi: z.array(z.string()).describe("Human-readable ABI array"),
  // Array of strings representing the ABI
  functionName: z.string().describe("Function name to call"),
  // Name of the function
  args: z.array(z.any()).optional().describe("Arguments for the function"),
  // Optional array of arguments
  target: z.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address").describe("Target contract address"),
  // Ethereum contract address
  chain: SupportedChainsSchema.describe("Blockchain identifier")
  // Chain identifier
});

// src/actions/actionContractInteraction.ts
var ContractInteractionAction = class {
  async interactWithContract(abi, functionName, args, target, chain, publicClient, walletClient, user) {
    elizaLogger2.info(`Interacting with contract at ${target} on ${chain}`);
...
var contractInteractionAction = {
  name: "CONTRACT_INTERACTION",
  description: "Interact with a smart contract function on any EVM-compatible chain using Gelato Relay. Supports both standard and ERC2771 calls.",
  ...
  handler: async (runtime, message, _state, _options, callback) => {
    try {
      const parsePrompt = (input) => {
        ...
        if (!abiMatch || !functionNameMatch || !targetMatch || !chainMatch) {
          throw new Error(
        ...
        return {
          ...
          user: userMatch ? userMatch[1] : void 0
          // Optional user for ERC2771
        };
      };
      const prompt = message.content.text;
      const parsedContent = parsePrompt(prompt);
      const validatedContent = ContractInteractionSchema.parse(parsedContent);
      const parsedAbi = parseAbi(validatedContent.abi);
      elizaLogger2.info(`Function: ${validatedContent.functionName}`);
      elizaLogger2.info(`Target: ${validatedContent.target}`);
      elizaLogger2.info(`Chain: ${validatedContent.chain}`);
      elizaLogger2.info(`Parsed ABI: ${JSON.stringify(parsedAbi)}`);
      if (parsedContent.user) {
        elizaLogger2.info(`User: ${parsedContent.user}`);
      }
    ...
    - Track Status: [View Task](${taskLink})`;
      ...
      const errorMessage = `\u274C Error in contract interaction: ${error instanceof Error ? error.message : "Unknown error"}`;
...
var index_default = gelatoPlugin;
export {
