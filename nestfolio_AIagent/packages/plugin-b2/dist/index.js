...
var FARM_ADDRESS = "0xd5B5f1CA0fa5636ac54b0a0007BA374A1513346e";

// src/utils/index.ts
var getTxReceipt = async (walletProvider2, tx) => {
  const publicClient = walletProvider2.getPublicClient();
  const receipt = await publicClient.waitForTransactionReceipt({
    hash: tx
  });
  return receipt;
};
var sendNativeAsset = async (walletProvider2, recipient, amount) => {
  const decimals = await walletProvider2.getDecimals(TOKEN_ADDRESSES["B2-BTC"]);
  const walletClient = walletProvider2.getWalletClient();
  const args = {
    account: walletProvider2.getAddress(),
    to: recipient,
...
var sendToken = async (walletProvider2, tokenAddress, recipient, amount) => {
  const decimals = await walletProvider2.getDecimals(tokenAddress);
  const publicClient = walletProvider2.getPublicClient();
  try {
    const { result, request } = await publicClient.simulateContract({
      account: walletProvider2.getAccount(),
...
var depositBTC = async (walletProvider2, farmAddress, amount) => {
  try {
...
var unstake = async (walletProvider2, farmAddress, amount) => {
  try {
    const BTC_PID = 0;
    const decimals = b2Network.nativeCurrency.decimals;
    const publicClient = walletProvider2.getPublicClient();
...
var withdraw = async (walletProvider2, farmAddress) => {
  try {
    const BTC_PID = 0;
    const publicClient = walletProvider2.getPublicClient();
    const { request } = await publicClient.simulateContract({
...
// src/environment.ts
var b2NetworkEnvSchema = z.object({
  B2_PRIVATE_KEY: z.string().min(1, "b2 network private key is required")
});
async function validateB2NetworkConfig(runtime) {
  try {
    const config = {
      B2_PRIVATE_KEY: runtime.getSetting("B2_PRIVATE_KEY") || process.env.B2_PRIVATE_KEY
    };
...
// src/templates/stake.ts
var stakeTemplate = `Respond with a JSON markdown block containing only the extracted values
...
// src/templates/unstake.ts
var unstakeTemplate = `Respond with a JSON markdown block containing only the extracted values
- Use null for any values that cannot be determined.
...
// src/templates/withdraw.ts
var withdrawTemplate = `Respond with a JSON markdown block containing only the extracted values
...
var WalletProvider = class {
  ...
  async get(runtime, _message, _state) {
    elizaLogger2.debug("walletProvider::get");
    try {
      ...
      if (this.account) {
        ...
        const walletProvider2 = await initWalletProvider(runtime);
...
`;
      ...
      elizaLogger2.debug("walletProvider::get output:", output);
      return output;
    } catch (error) {
      elizaLogger2.error("Error in b2 wallet provider:", error);
      return null;
    }
  }
};
...
var walletProvider = {
  async get(runtime, _message, _state) {
    ...
    try {
      const walletProvider2 = await initWalletProvider(runtime);
      const account = walletProvider2.getAccount();
      let output = "# Wallet Balances\n\n";
      output += "## Wallet Address\n\n";
...
`;
      output += "## Latest Token Balances\n\n";
      for (const [token, address] of Object.entries(TOKEN_ADDRESSES)) {
        const decimals = await walletProvider2.getDecimals(address);
        const balance = await walletProvider2.getTokenBalance(
          address,
          account.address
        );
        output += `${token}: ${formatUnits(balance, decimals)}
...
var TransferAction = class {
  ...
  async transfer(params) {
    try {
      ...
      elizaLogger3.error(`Transfer failed: ${error.message}`);
  ...
  async buildTransferDetails(state, runtime) {
...
var transferAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger3.debug("Starting SEND_TOKEN handler...");
    if (!state) {
      state = await runtime.composeState(message);
    } else {
      state = await runtime.updateRecentMessageState(state);
    }
    elizaLogger3.debug("Transfer action handler called");
    const walletProvider2 = await initWalletProvider(runtime);
    const action = new TransferAction(walletProvider2);
    const paramOptions = await action.buildTransferDetails(
      state,
      runtime
    );
    elizaLogger3.debug("Transfer paramOptions:", paramOptions);
...
var StakeAction = class {
  ...
  async stake(params) {
    try {
      ...
      elizaLogger4.error(`Stake failed: ${error.message}`);
  ...
  async buildStakeDetails(state, runtime) {
    const context = composeContext2({
      state,
      template: stakeTemplate
    });
    const stakeDetails = await generateObjectDeprecated2({
      runtime,
      context,
...
var stakeAction = {
  ...
  description: "stake B2-BTC.",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger4.debug("Starting STAKE handler...");
    if (!state) {
      state = await runtime.composeState(message);
    } else {
      state = await runtime.updateRecentMessageState(state);
    }
    elizaLogger4.debug("stake action handler called");
    const walletProvider2 = await initWalletProvider(runtime);
    const action = new StakeAction(walletProvider2);
    const paramOptions = await action.buildStakeDetails(
      state,
      runtime
    );
    elizaLogger4.debug("Stake paramOptions:", paramOptions);
...
var UnstakeAction = class {
  ...
  async unstake(params) {
    try {
      ...
      elizaLogger5.error(`Unstake failed: ${error.message}`);
  ...
  async buildUnstakeDetails(state, runtime) {
    const context = composeContext3({
      state,
      template: unstakeTemplate
    });
    const unstakeDetails = await generateObjectDeprecated3({
      runtime,
      context,
...
var unstakeAction = {
  ...
  description: "unstake B2-BTC.",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger5.debug("Starting UNSTAKE handler...");
    if (!state) {
      state = await runtime.composeState(message);
    } else {
      state = await runtime.updateRecentMessageState(state);
    }
    elizaLogger5.debug("unstake action handler called");
    const walletProvider2 = await initWalletProvider(runtime);
    const action = new UnstakeAction(walletProvider2);
    const paramOptions = await action.buildUnstakeDetails(
      state,
      runtime
    );
    elizaLogger5.debug("Unstake paramOptions:", paramOptions);
...
var WithdrawAction = class {
  ...
  async withdraw(_params) {
    try {
      ...
      elizaLogger6.log(`Withdraw failed: ${error.message}`);
  ...
  async buildWithdrawDetails(state, runtime) {
    const context = composeContext4({
      state,
      template: withdrawTemplate
    });
    const withdrawDetails = await generateObjectDeprecated4({
      runtime,
      context,
...
var withdrawAction = {
  ...
  description: "withdraw B2-BTC.",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger6.debug("Starting WITHDRAW handler...");
    ...
    elizaLogger6.debug("withdraw action handler called");
    const walletProvider2 = await initWalletProvider(runtime);
    const action = new WithdrawAction(walletProvider2);
    const paramOptions = await action.buildWithdrawDetails(
      currentState,
      runtime
    );
    elizaLogger6.debug("Withdraw paramOptions:", paramOptions);
