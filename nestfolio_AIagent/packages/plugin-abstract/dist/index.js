...
var validatedTransferSchema = z.object({
  tokenAddress: z.string().refine(isAddress3, { message: "Invalid token address" }),
  recipient: z.string().refine(isAddress3, { message: "Invalid recipient address" }),
  amount: z.string(),
  useAGW: z.boolean()
});
...
var validatedSchema = z.object({
  tokenAddress: z.string().refine(isAddress4, { message: "Invalid token address" }),
  walletAddress: z.string().refine(isAddress4, { message: "Invalid token address" })
});
...
var validatedSchema2 = z.object({
  name: z.string().min(1, "Name is required"),
  symbol: z.string().min(1, "Symbol is required").max(5, "Symbol must be 5 characters or less"),
  initialSupply: z.string().refine((val) => !Number.isNaN(Number(val)) && Number(val) > 0, {
