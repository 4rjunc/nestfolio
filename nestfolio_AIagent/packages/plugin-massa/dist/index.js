...
import { Account, Web3Provider, CHAIN_ID, MAINNET_TOKENS, BUILDNET_TOKENS, Address, MRC20, parseUnits, MNS, MNS_CONTRACTS, PublicApiUrl } from '@massalabs/massa-web3';
...
// src/enviroment.ts
var massaEnvSchema = z.object({
  MASSA_PRIVATE_KEY: z.string().min(1, "Massa private key is required"),
  MASSA_RPC_URL: z.string().min(1, "Massa RPC URL is required")
});
async function validateConfig(runtime) {
  try {
    const config = {
      MASSA_PRIVATE_KEY: runtime.getSetting("MASSA_PRIVATE_KEY") || process.env.MASSA_PRIVATE_KEY,
      MASSA_RPC_URL: runtime.getSetting("MASSA_RPC_URL") || process.env.MASSA_RPC_URL || PublicApiUrl.Mainnet
    };
...
var getMnsTarget = async (provider, name) => {
  const { chainId } = await provider.networkInfos();
  const mnsContract = new MNS(
    provider,
    chainId === CHAIN_ID.Mainnet ? MNS_CONTRACTS.mainnet : MNS_CONTRACTS.buildnet
  );
...
var transferTemplate = (tokens) => `Respond with a JSON markdown block containing only the extracted values. Use null for any values that cannot be determined.
...
${Object.entries(tokens).map(([name, address]) => `- ${name}: ${address}`).join("\n")}
...
var transfer_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    const isValid2 = isTransferContent(content);
    if (!isValid2) {
...
var index_default = massaPlugin;

export { index_default as default, massaPlugin };
