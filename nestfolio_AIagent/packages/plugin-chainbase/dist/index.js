// src/actions/retrieveTokenBalance.ts
import {
...
// src/libs/chainbase.ts
import { elizaLogger } from "@elizaos/core";

// src/constants.ts
var CHAINBASE_API_URL_ENDPOINT = "https://api.chainbase.com";
...
async function getTokenBalances(params) {
  try {
    const apiKey = getChainbaseApiKey();
...
var responsePrompt = (result, query) => `You are a blockchain data analyst. Your task is to analyze and present the query results from Chainbase in a clear and informative way.
...
1. **Format the Response**: Present the results in a structured format with emojis for better readability:
   - \u{1F4CA} Query Results (with user's original query)
   - \u{1F50D} SQL Query used
   - \u{1F4CB} Data in table format
   - \u{1F4C8} Brief analysis of the results
...
\u{1F4CA} Query Results for: [user's query]
...
\u{1F4C8} Analysis: [Brief analysis of the results]
...
var retrieveTokenBalanceTemplate = `
...
{{recentMessages}}
`;
var formatTokenBalancePrompt = (tokens, address) => `
...
var retrieveTokenBalance = {
  ...
  validate: async (runtime, _message) => {
    elizaLogger2.log("Validating runtime for RETRIEVE_TOKEN_BALANCE...");
    return !!(runtime.character.settings.secrets?.CHAINBASE_API_KEY || process.env.CHAINBASE_API_KEY);
  },
  handler: async (runtime, message, state, _options, callback) => {
    try {
      ...
      elizaLogger2.log("Querying token balances:", {
        chain_id,
        address,
        contract_address
      });
      ...
      const processedTokens = tokens.map((token) => ({
        ...token,
        balance: token.balance ? new big_default(Number.parseInt(token.balance, 16).toString()).div(new big_default(10).pow(token.decimals)).toFixed(18) : "0"
      }));
      if (processedTokens.length > 0) {
        ...
        callback({
          text: `\u{1F4AB} No token balances found for address ${address}`
        });
      }
    } catch (error) {
      elizaLogger2.error("Error in retrieveTokenBalance:", error.message);
      callback({
...
// src/actions/queryData.ts
import {
  elizaLogger as elizaLogger3,
  generateText as generateText2,
  ModelClass as ModelClass2
} from "@elizaos/core";
var QUERY_PREFIX = "query onchain data:";
var queryBlockChainData = {
  name: "QUERY_BLOCKCHAIN_DATA",
  similes: ["ANALYZE_BLOCKCHAIN", "GET_CHAIN_DATA", "QUERY_ONCHAIN_DATA"],
  description: "Query blockchain data using natural language starting with 'query onchain data:'",
  validate: async (runtime, _message) => {
    elizaLogger3.log("Validating runtime for QUERY_BLOCKCHAIN_DATA...");
    return !!(runtime.character.settings.secrets?.CHAINBASE_API_KEY || process.env.CHAINBASE_API_KEY);
  },
  handler: async (runtime, message, _state, _options, callback) => {
    try {
      ...
      const queryText = message.content.text.slice(
        message.content.text.toLowerCase().indexOf(QUERY_PREFIX) + QUERY_PREFIX.length
      ).trim();
      if (!queryText) {
        callback({
      ...
      elizaLogger3.error("Error in queryChainbase action:", error);
      callback({
